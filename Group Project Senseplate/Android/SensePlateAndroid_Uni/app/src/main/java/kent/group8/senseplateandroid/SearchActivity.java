...
public class SearchActivity extends AppCompatActivity implements BackgroundWorker.TaskListener {
    ...
    public void search(String searchedItem) {
        ...
        String url = "https://api.edamam.com/api/food-database/v2/parser?ingr=" + searchedItem.replace(" ", "%20") + "&nutrition-type=logging&category=generic-foods&catergoryLabel=food&app_id=a02bcc70&app_key=4e0c1dbdbd5c15af5c8b9501c9432467";
    ...
    public void parseSearch(String result) throws JSONException {
        ...
        String foodID = "", foodName = "", brand = "", amount = "", kcal = "", protein = "", fat = "", carbs = "";
        ...
        for (int i = 0; i < jsonObject.length(); i++) {
            ...
            try {
                JSONArray amountObject = result1.getJSONArray("measures");
                for (int j = 0; j < amountObject.length(); j++) {
                    JSONObject result2 = amountObject.getJSONObject(j);
                    String servingType = result2.getString("label");
                    if (servingType.equals("Serving")) {
                        amount = result2.getString("weight");
                    }
                }
            } catch (Exception e) {
            ...
            searchList.add(new SearchItem(foodID, WordUtils.capitalizeFully(foodName), amount, brand, kcal, protein, fat, carbs));
        }
